...
class PurePursuit{
	...
	public:
		PurePursuit(ros::NodeHandle& n, string file_name){
		    n.param<float>("/pure_pursuit/LOOK_AHEAD_DIST", _LOOK_AHEAD_DIST,2.0);
            n.param<float>("/pure_pursuit/P_gain", _P_gain,0.1);
            n.param<float>("/pure_pursuit/velocity", _velocity,1);
...
//			_odom_sub = n.subscribe("pf/pose/odom", 1, &PurePursuit::odom_callback, this);
            ...
            _nav_pub = n.advertise<ackermann_msgs::AckermannDriveStamped>("/drive", 1);
//             _nav_pub = n.advertise<ackermann_msgs::AckermannDriveStamped>("/vesc/high_level/ackermann_cmd_mux/input/nav_1",1);

			_setpoint_pub = n.advertise<visualization_msgs::Marker>("/set_point", 1);
		...
		if(_waypoints.size() !=0){
			...
			publish_visual(_current_ind);
			

		}
	}

		void publish_visual(int _current_ind)
		{
                ...
                p.pose.position.x = _waypoints[_current_ind].getX();
                p.pose.position.y = _waypoints[_current_ind].getY();
...
int main(int argc, char ** argv) {
    ...
    string file_name = "/home/robert/catkin_ws/src/pure_pursuit_tracking/src/yuwei_wp.csv";
